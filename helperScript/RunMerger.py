...
for key,item in exclusion_dict.items():
    ...
    if (chamber%2==0 and key[-1]!="L") or (chamber%2==1 and key[-1]!="S"): ## If even must be Long, odd must be Short
        ...
        sys.exit(0)
    print(f"Excluding {key} from run(s):\t {str(exclusion_dict[key])[1:-1]}")
## safety check the exclusion list
print(f"########\n")
...
try:
    ...
    print(f"Failed to import csv for one or more input files in {inputs}\nExiting ..")
    sys.exit(0)
try:
    df_list_byVFAT = [ pd.read_csv(i, sep=',') for i in inputs_byVFAT]
except IOError:
    print(f"Failed to import csv for one or more input files in {inputs_byVFAT}\nExiting ..")
    sys.exit(0)
if DLE:
    try:
        df_list_byDLE = [ pd.read_csv(i, sep=',') for i in inputs_byDLE]
    except IOError:
        print(f"Failed to import csv for one or more input files in {inputs_byDLE}\nExiting ..")
        sys.exit(0)
...
print(f"Merging {len(df_list)} files for standard procedure")
print(f"Merging {len(df_list_byVFAT)} files for efficiency by VFAT")
if DLE: print(f"Merging {len(df_list_byDLE)} files for efficiency by DLE")
